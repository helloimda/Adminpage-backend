...
const searchGfs = (searchParams) => {
    return new Promise((resolve, reject) => {
        const conditions = [];
        const values = [];

        if (searchParams.de_idx) {
            conditions.push('dep.de_idx = ?');
            values.push(searchParams.de_idx);
        }
        if (searchParams.brand_name) {
            conditions.push('brand2.brand_rname = ?');
            values.push(searchParams.brand_name);
        }
        if (searchParams.de_name) {
            conditions.push('dep.de_name = ?');
            values.push(searchParams.de_name);
        }
        if (searchParams.snu) {
            conditions.push('cr.snu = ?');
            values.push(searchParams.snu);
        }
        if (searchParams.dcolor) {
            conditions.push('(cr.dcolor1 = ? OR cr.dcolor2 = ?)');
            values.push(searchParams.dcolor, searchParams.dcolor);  // 두 컬럼 중 하나와 일치하는 경우
        }
        ...
        query += `
            ...
            AND DATE_ADD(DATE_ADD(CONCAT(crs.dt, SUBSTR(NOW(), 11, 10)), INTERVAL -10 HOUR), INTERVAL 1 DAY) > NOW()
            ORDER BY 
                dep.de_idx ASC,  -- de_idx로 오름차순 정렬
                CASE 
                    WHEN crs.cgs_idx IS NULL OR crs.cnt = 0 THEN 0 
                    ELSE 1 s
                END DESC,
